...
Code: <select name="code" id="search_code" style="width:15%;height:30px;">
  ...
  <div id="outer-container">
      ...
      <table id="emp_table" border="0" class="resultGridTable" style="width: auto;margin-top: 1%;font-size:10px;">
        <tr class="tr_header" style="background-color:#3385ff;font-family:Tahoma, Geneva, sans-serif;font-size: 12px;">
          ...
          *
          ...
          IFNULL(LastWIP,0) AS LastWIP,
          IFNULL(returnrrQty,0) AS returnrrQty,
          ...
          pUSD AS proUSD,
          SUM(Qty) AS totalQty,
          (CASE WHEN IFNULL(ddQty,0) < IFNULL(LastWIP,0) THEN ( (SUM(Qty) -  IFNULL(LastWIP,0)) - IFNULL(LasttoutQty,0) ) +  IFNULL(returnrrQty, 0)   
               WHEN IFNULL(LastWIP,0) < IFNULL(ddQty,0) THEN ( (SUM(Qty) -  IFNULL(ddQty,0)) - IFNULL(LasttoutQty,0) ) + IFNULL(returnrrQty, 0)   
               WHEN IFNULL(LastWIP,0) = IFNULL(ddQty,0) THEN ( (SUM(Qty) -  IFNULL(LastWIP,0)) - IFNULL(LasttoutQty,0)) + IFNULL(returnrrQty, 0) 
               END) AS  balancecQty,
          ( ( (SUM(Qty) - IFNULL(ddQty, 0)) - IFNULL(LasttoutQty,0)) + IFNULL(returnrrQty, 0)  ) AS currentcQty,
   ...
   AND PoMst.Branch=tblTransfer.transfer_from 
     LEFT JOIN( SELECT  t.transfer_date AS tttransfer_date,t.BrandName AS ttBrandName,t.Commodity AS ttCommodity,
  t.code AS ttcode,t.LasttoutQty AS LasttoutQty,COUNT(*) AS tovisits,t.transfer_from AS tttransfer_from,t.tRate as tRate FROM (SELECT ANY_VALUE(transfer_date) AS transfer_date,SUM(Qty) AS LasttoutQty,SUM(transferUSD) AS tRate,ANY_VALUE(PL) AS PL,BrandName,Commodity,transfer_from,code FROM transfer GROUP BY Commodity,BrandName,code,transfer_from
  ) AS t JOIN product pt ON pt.Purchase_order_No = t.PL AND pt.BrandName = t.BrandName AND pt.Commodity = t.Commodity AND pt.Branch = t.transfer_from AND pt.code=t.code";
    ...
    FROM job USE INDEX (Location,Job_date,BrandName,Commodity,code)";
      ...
      //end transfer in
         ...
         // new
          ...
          LEFT JOIN( SELECT  d.Delivery_date AS ddDelivery_date,d.BrandName AS ddBrandName,d.Commodity AS ddCommodity,
  d.code AS ddcode,d.ddQty AS ddQty,COUNT(*) AS visits,d.Location AS ddLocation,d.pRate as pRate FROM(SELECT ANY_VALUE(Delivery_date) AS Delivery_date,SUM(Qty) AS ddQty,
  SUM(deliveryUSD) AS pRate,ANY_VALUE(PL) AS PL,BrandName,Commodity,Location,code FROM delivery GROUP BY Commodity,BrandName,code,Location
  ) AS d JOIN product pd ON pd.Purchase_order_No = d.PL AND pd.BrandName = d.BrandName AND pd.Commodity = d.Commodity AND pd.Branch = d.Location AND pd.code=d.code";
          ...
          AND PoMst.Commodity=tblDeli.dCommodity          
          AND PoMst.code=tblDeli.dcode
          AND PoMst.Branch=tblDeli.dLocation LEFT JOIN( SELECT  d.Delivery_date AS ddDelivery_date,d.BrandName AS ddBrandName,d.Commodity AS ddCommodity,
  d.code AS ddcode,d.ddQty AS ddQty,COUNT(*) AS visits,d.Location AS ddLocation,d.pRate as pRate FROM(SELECT ANY_VALUE(Delivery_date) AS Delivery_date,SUM(Qty) AS ddQty,
  SUM(deliveryUSD) AS pRate,ANY_VALUE(PL) AS PL,BrandName,Commodity,Location,code FROM delivery GROUP BY Commodity,BrandName,code,Location
  ) AS d JOIN product pd ON pd.Purchase_order_No = d.PL AND pd.BrandName = d.BrandName AND pd.Commodity = d.Commodity AND pd.Branch = d.Location AND pd.code=d.code"; 
...
//   LEFT JOIN (SELECT ANY_VALUE(d.Delivery_date) AS ddDelivery_date, d.BrandName AS ddBrandName,d.Commodity AS ddCommodity,d.code AS ddcode,SUM(d.Qty) AS ddQty,d.Location AS ddLocation,SUM(pd.USD * d.Qty) as pRate
        //   FROM delivery d  LEFT JOIN product pd ON pd.Purchase_order_No = d.PL AND pd.BrandName = d.BrandName AND pd.Commodity = d.Commodity AND pd.Branch = d.Location AND pd.code=d.code";
          ...
          $sqlA .=" GROUP BY d.Commodity,d.BrandName,d.Location,d.code ORDER BY COUNT(*) DESC) tblDeliLast 
          ...
          ,GROUP_CONCAT(Remarks_item SEPARATOR ' ') AS rRemarks  FROM returns USE INDEX (department,Return_date,BrandName,Commodity,code)"; 
           }else{
          ...
          AND PoMst.code=tblRet.rcode
             AND PoMst.Branch=tblRet.rDepartment  LEFT JOIN( SELECT  r.Return_date AS rrReturn_date,r.BrandName AS rrBrandName,r.Commodity AS rrCommodity,
      r.code AS rrcode,r.returnrrQty AS returnrrQty,COUNT(*) AS rvisits,r.department AS rrDepartment,r.rRate as rRate  FROM(SELECT ANY_VALUE(Return_date) AS Return_date,
      SUM(Qty) AS returnrrQty,SUM(returnUSD) as rRate,ANY_VALUE(PL) AS PL,BrandName,Commodity,department,code FROM returns GROUP BY Commodity,BrandName,code,department
  ) AS r JOIN product pr ON pr.Purchase_order_No = r.PL AND pr.BrandName = r.BrandName AND pr.Commodity = r.Commodity AND pr.Branch = r.department AND pr.code=r.code";
        ...
        //   LEFT JOIN product pr ON pr.Purchase_order_No = r.PL AND pr.BrandName = r.BrandName AND pr.Commodity = r.Commodity AND pr.Branch = r.department AND pr.code=r.code";
          ...
          AND PoMst.Branch=tblRet.rDepartment
            AND PoMst.Branch=tblRet.rDepartment 
            LEFT JOIN( SELECT  ANY_VALUE(r.Return_date) AS rrReturn_date,r.Commodity AS rrCommodity,r.BrandName AS rrBrandName,r.code AS rrcode,r.returnrrQty AS returnrrQty,
            COUNT(*) AS visits,r.department AS rrDepartment,r.rRate as rRate FROM (SELECT ANY_VALUE(Return_date) AS Return_date,SUM(Qty) AS returnrrQty,
 ANY_VALUE(PL) AS PL,SUM(returnUSD) as rRate,BrandName,Commodity,department,code FROM returns GROUP BY Commodity,BrandName,code,department
  ) AS r JOIN product pr ON pr.Purchase_order_No = r.PL AND pr.BrandName = r.BrandName AND pr.Commodity = r.Commodity AND pr.Branch = r.department AND pr.code=r.code";
