        ...
        if($count==1)
             {
              ...
              echo "<script type='text/javascript'>window.top.location='detail.php?Customer=$okyaku&Return_no=$Return_no&DO_No=$DO_No';</script>";exit;              
                  ...
                  $sqlproduct="SELECT balanceWIP,balanceReturn,balanceQty,USD,balanceUSD,Qty,balanceDelivery,totalUSD FROM product Where BrandName = '$NBrandName' and Commodity ='$NCommodity' and Purchase_order_No ='$PL' AND Branch LIKE '%{$department}%'";
                  $resultproduct=mysqli_query($con ,$sqlproduct); 
                  $product = $resultproduct->fetch_assoc();
                  $returns=$product['balanceReturn']+$Qty;

                 $sqls = " UPDATE product SET balanceReturn='$returns' WHERE BrandName =  '$NBrandName' AND Commodity LIKE '%{$NCommodity}%'  AND Purchase_order_No LIKE '%{$PL}%' AND Branch LIKE '%{$department}%'";

                  mysqli_query($con,$sqls);

                  $sqlproducts="SELECT balanceWIP,balanceReturn,balanceQty,USD,balanceUSD,Qty,balanceDelivery,totalUSD FROM product Where BrandName = '$NBrandName' and Commodity ='$NCommodity' and Purchase_order_No ='$PL' AND Branch LIKE '%{$department}%'";
                  ...
                  $sqls = " UPDATE product SET balanceUSD='$balanceUSD',balanceQty='$totalbalance' WHERE BrandName =  '$NBrandName' AND Commodity LIKE '%{$NCommodity}%'  AND Purchase_order_No LIKE '%{$PL}%' AND Branch LIKE '%{$department}%'";

                  mysqli_query($con,$sqls);        
                  
                  $sqlsAA = " UPDATE product SET currentQty='$currentQty' WHERE BrandName =  '$NBrandName' AND Commodity LIKE '%{$NCommodity}%'  AND Purchase_order_No LIKE '%{$PL}%' AND Branch LIKE '%{$department}%'";

                  mysqli_query($con,$sqlsAA);                    
               
                  $sqldelivery="SELECT returnQty FROM delivery Where Customer='$okyaku' AND BrandName = '$NBrandName' and Commodity ='$NCommodity' and PL ='$PL' AND Location LIKE '%{$department}%' AND DO_No LIKE '%{$DO_No}%'";
