...
fputcsv($output,  array('Branch','BrandName','Commodity','Total Input','Total WIP','Total Delivery','Total Return','Remark','Total Stock Balance For Sale','Actual Total Stock Balance','Total Input USD','Total Output USD','Balance USD'));  
}
else{
fputcsv($output,  array('Branch','BrandName','Commodity','Total Input','Total WIP','Total Delivery','Total Return','Remark','Total Stock Balance For Sale','Actual Total Stock Balance'));    
}
...
if(isset($_SESSION["InStock_namelist"])){
 ...
 $Branches = $_SESSION['Branch'];     
         ...
         IFNULL(returnrQty,0) AS rQty,
          (CASE WHEN IFNULL(ddQty,0) < IFNULL(LastWIP,0) THEN (IFNULL(ANY_VALUE(SUM(Qty)),0) -  IFNULL(LastWIP,0)) +  IFNULL(returnrrQty, 0)   
               WHEN IFNULL(LastWIP,0) < IFNULL(ddQty,0) THEN (IFNULL(ANY_VALUE(SUM(Qty)),0) -  IFNULL(ddQty,0)) + IFNULL(returnrrQty, 0)   
               WHEN IFNULL(LastWIP,0) = IFNULL(ddQty,0) THEN (IFNULL(ANY_VALUE(SUM(Qty)),0) -  IFNULL(LastWIP,0)) + IFNULL(returnrrQty, 0) 
               END) AS  balancecQty,
          ( ( IFNULL(ANY_VALUE(SUM(Qty)), 0) - IFNULL(ddQty, 0)) + IFNULL(returnrrQty, 0)  ) AS currentcQty";
          
          if ($Authority == 'Accountant') {
  $sqlA .=",SUM(totalUSD) AS totalUSDs,
          IFNULL(pRate, 0) AS balancetotalUSD,
          SUM(totalUSD) - IFNULL(pRate, 0) AS netUSDs"; 
         }
        $sqlA=$sqlA."
          FROM 
         ...
         // new
          ...
          LEFT JOIN (SELECT ANY_VALUE(d.Delivery_date) AS ddDelivery_date, d.BrandName AS ddBrandName,d.Commodity AS ddCommodity,SUM(d.Qty) AS ddQty,d.Location AS ddLocation,SUM(pd.rate * d.Qty) as pRate
          ...
          LEFT JOIN (SELECT ANY_VALUE(r.Return_date) AS rrReturn_date,r.Commodity AS rrCommodity,r.BrandName AS rrBrandName,SUM(r.Qty) AS returnrrQty,r.department AS rrDepartment FROM returns r USE 
          INDEX (department,Return_date,BrandName,Commodity)";
       ...
       if(!empty($_POST['startDate'])  && !empty($_POST['endDate'])){
         ...
         // } 
          if (!empty($_POST['keyword'])) {
           $keyword = $_POST['keyword'];
           $sqlA =$sqlA." AND ( BrandName = '$keyword' OR Commodity = '$keyword' OR branch = '$keyword' )";
         }
          $sqlA = $sqlA. " GROUP BY
          PoMst.BrandName,
          PoMst.Commodity,
