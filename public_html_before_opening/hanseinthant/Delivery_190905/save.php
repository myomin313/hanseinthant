...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
      ...
      $sqlW="SELECT currentQty FROM product Where BrandName = '$BrandName' and Commodity ='$Commoditys' and Branch ='$Location' and Purchase_order_No ='$txtHintss'";
      $resultW=mysqli_query($con ,$sqlW); 
      $rowsW = $resultW->fetch_assoc();
    //   echo $rowsW['currentQty'];
    //   echo $Qty;
      
      
     if($Qty > $rowsW['currentQty']){
                mysqli_rollback($con);
    ...
    //   echo $sql;
      ...
      $sql = " UPDATE product SET balanceDelivery ='$balanceDeliverys',balanceUSD ='$balanceUSDss' Where BrandName = '$BrandName' and Commodity ='$Commoditys' and Branch ='$Location' and Purchase_order_No ='$txtHintss'"; 
      // echo $sql;
      mysqli_query($con ,$sql);
      ...
      $i += 1;
             ...
             $sql = 'INSERT INTO delivery(Location,Customer,Commodity,BrandName,Type,PL,Qty,Delivery_date,DO_No,JOB_No,Remark_item,returnQty,Creator,CreateTime) VALUES '.implode
      (',', $row_data);
    //   echo $sql;
         mysqli_query($con ,$sql);
         mysqli_commit($con);
