...
$select ="SELECT Australia,Rate_date FROM rate";
 $results=mysqli_query($con ,$select); 
 $rows = $results->fetch_assoc();

 while($row = mysqli_fetch_array($results)){
 
     $USD = (float) str_replace(',','', $row['Australia']) ;
     $Rate_date = $row['Rate_date'];

      $sql = "UPDATE product SET totalUSD = totalRate * '$USD' WHERE Packing_date='$Rate_date' AND  Rate_Name='Australia'";
