...
include_once('../header.php');
     ...
     /// var newstring = string.replace(/[&\/\\#,+()$~%.':*?<>{}]/g,"\\$&");
          ...
          window.location.href = "list.php?startDate="+startDate+"&endDate="+endDate+"&keyword="+keywords;
        });  
            });
          </script> 
      ...
      <table id="emp_table" border="0" class="resultGridTable" style="width: auto;margin-top: 1%;font-size:10px;">
        <tr class="tr_header" style="background-color:#3385ff;font-family:Tahoma, Geneva, sans-serif;font-size: 12px;">
          ...
          FROM job j USE INDEX (Location,Job_date,BrandName,Commodity)";  
           ...
           if(!empty($_GET['startDate'])  && !empty($_GET['endDate'])) {
             ...
             if( $Branches <> 'All'){
               ...
               $sqlA .="  AND j.Job_date <= '$toDate' GROUP BY j.BrandName,j.Commodity,j.Location) tblJobLast
         ...
         // new
          ...
          $sqlA .=" AND d.Delivery_date <= '$toDate' GROUP BY d.Commodity,d.BrandName,d.Location ) tblDeliLast 
