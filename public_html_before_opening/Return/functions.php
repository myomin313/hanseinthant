...
include_once('../config.php');
 if(isset($_POST["Import"])){
		...
		$filename=$_FILES["file"]["tmp_name"];		
		 ...
		 if($_FILES["file"]["size"] > 1)
		 {
	        ...
	        $sql = "INSERT into returns (id,Return_date,Return_no,department,Customer,DO_No,BrandName,Commodity,Qty,return,Type,Remarks_item,Creator,CreateTime ) 
 ...
 fputcsv($output,  array('id','Return date','Return no','To','Customer','DO No','Code','BrandName','Commodity','Qty','returnUSD','Type','Remark_item','Creator','CreateTime'));  
...
$sWhere = "";
$sqls=" SELECT returns.id,returns.Return_date,returns.Return_no,branch.branch,customer.CustomerName,returns.Do_No,code.code,brandname.brandname as BrandName,commodity.commodity as Commodity,returns.Qty,returns.returnUSD,returns.Type,returns.Remarks_item,returns.Creator,returns.CreateTime FROM returns 
                  LEFT JOIN customer
                  ON returns.Customer= customer.UserId
                  LEFT JOIN code
                  ON returns.code= code.id
                ...
                if(isset($_POST['Return_No']) && $_POST['Return_No'] != ''){
                  $Return_No=$_POST['Return_No'];
                 $sWhere =$sWhere." AND returns.Return_No='$Return_No'";
               }
               if(isset($_POST['department'])  && $_POST['department'] != ''){
                 $department=$_POST['department'];
                 $sWhere =$sWhere." AND returns.department='$department'";
               }
               if(isset($_POST['CustomerName'])  && $_POST['CustomerName'] != ''){
                 $CustomerName=$_POST['CustomerName'];
                 $sWhere =$sWhere." AND returns.Customer='$CustomerName'";
               }
               if(isset($_POST['BrandName']) && $_POST['BrandName'] != ''){
                $BrandName=$_POST['BrandName'];
                $sWhere =$sWhere." AND returns.BrandName='$BrandName'";
              }
              if(isset($_POST['Commodity']) && $_POST['Commodity'] != ''){
                 $Commodity=$_POST['Commodity'];
                $sWhere =$sWhere." AND returns.Commodity='$Commodity'";
              }
              if(isset($_POST['code'])  && $_POST['code'] != ''){
                 $code=$_POST['code'];
                $sWhere =$sWhere." AND returns.code='$code'";
              }
              if(isset($_POST['DO_No']) && $_POST['DO_No'] != ''){
                $DO_No=$_POST['DO_No'];
                $sWhere =$sWhere." AND returns.DO_No='$DO_No'";
              }
                //  if(!empty($_POST['keyword'])){
                //    $keyword = $_POST['keyword'];
