...
$query ="SELECT DISTINCT brandname.brandname as BrandName,product.Branch FROM product  LEFT JOIN brandname
                  ON product.BrandName= brandname.id WHERE brandname.brandname like '".$keyword. "%' and Branch='Shwe Pyi Thar Store'  ORDER BY brandname.brandname LIMIT 0,15 ";
...
<li onClick="set_Brand_items('<?php echo htmlspecialchars($brands, ENT_QUOTES); ?>');"><?php echo  mb_convert_encoding($row["BrandName"], 'UTF-8', 'HTML-ENTITIES'); ?></li>
<?php } ?>
</ul>
<?php } 
...
<li onClick="set_supplier_items('<?php echo htmlspecialchars($Suppliers, ENT_QUOTES); ?>');"><?php echo $SupplierA; ?></li>
<?php } ?>
</ul>
<?php } 
