...
if(isset($_POST["Export"])){
 ...
 $filename = "Delivery list/" . date('Y-m-d') . ".csv";
...
// output headers so that the file is downloaded rather than displayed
 ...
 fputcsv($output,  array('id','Delivery Date','DO No','Location','Customer','Code','BrandName','Commodity','Qty','Delivery USD','Return Qty','Type','Remark_item','PL','Transfer_No','Creator','CreateTime'));
...
// $sql = "SELECT id,Delivery_date,DO_No,Location,Customer,BrandName,Commodity,Qty,Type,Remark_item,PL,Creator,CreateTime from delivery ";
   $sWhere = "";
   $sql=" SELECT delivery.id,delivery.Delivery_date,delivery.DO_No,branch.branch,customer.customerName,code.code,brandname.brandname as BrandName,commodity.commodity as Commodity,delivery.Qty,delivery.deliveryUSD,delivery.returnQty,delivery.Type,delivery.Remark_item,delivery.PL,delivery.transfer_no,delivery.Creator,delivery.CreateTime 
          FROM delivery 
          LEFT JOIN customer ON customer.UserId= delivery.Customer
          LEFT JOIN code ON code.id= delivery.code
          JOIN branch ON delivery.Location= branch.id
          ...
          JOIN commodity ON delivery.Commodity= commodity.id"; 
                  ...
                  $fromDate = date("Y-m-d", strtotime($_POST['fromDate']));
                   ...
                   $sWhere = $sWhere ." AND delivery.Delivery_date BETWEEN '$fromDate'  AND '$toDate'";
                   }
                if(isset($_POST['DO_No']) && $_POST['DO_No'] != ''){
                  $DO_No=$_POST['DO_No'];
                 $sWhere =$sWhere." AND delivery.DO_No='$DO_No'";
               }
               if(isset($_POST['Branch'])  && $_POST['Branch'] != ''){
                 $Branch=$_POST['Branch'];
                 $sWhere =$sWhere." AND delivery.Location='$Branch'";
               }
               if(isset($_POST['CustomerName'])  && $_POST['CustomerName'] != ''){
                 $CustomerName=$_POST['CustomerName'];
                 $sWhere =$sWhere." AND delivery.Customer='$CustomerName'";
               }
               if(isset($_POST['BrandName']) && $_POST['BrandName'] != ''){
                $BrandName=$_POST['BrandName'];
                $sWhere =$sWhere." AND delivery.BrandName='$BrandName'";
              }
              if(isset($_POST['Commodity']) && $_POST['Commodity'] != ''){
                $Commodity=$_POST['Commodity'];
                $sWhere =$sWhere." AND delivery.Commodity='$Commodity'";
              }
              if(isset($_POST['code'])  && $_POST['code'] != ''){
                $code=$_POST['code'];
                $sWhere =$sWhere." AND delivery.code='$code'";
              }
              if(isset($_POST['PL']) && $_POST['PL'] != ''){
                $PL=$_POST['PL'];
                $sWhere =$sWhere." AND delivery.PL='$PL'";
              }
