...
if ($Authority == 'Accountant') {
  ...
  SUM(totalUSD) - SUM(balanceUSD) AS netUSDs
  ";
}
...
LEFT JOIN (SELECT BrandName AS jBrandName,Commodity AS jCommodity,Location AS jLocation,SUM(WIP) AS wipQTY,GROUP_CONCAT(Remarks_Item  SEPARATOR '   ') AS jRemarks
FROM job"; 
if ($Branches <> 'All') {
...
LEFT JOIN (SELECT  BrandName AS dBrandName,Commodity AS dCommodity,Location AS dLocation,SUM(Qty) AS deliQTY,GROUP_CONCAT(Remark_item  SEPARATOR '   ') AS dRemarks
FROM delivery";  
...
LEFT JOIN (SELECT Commodity AS rCommodity,BrandName AS rBrandName,department AS rDepartment,SUM(Qty) AS retQty,GROUP_CONCAT(Remarks_item  SEPARATOR '   ') AS rRemarks
FROM returns";  
if ($Branches <> 'All') {
