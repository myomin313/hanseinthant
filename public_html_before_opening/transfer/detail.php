...
<script>
    function showPl(rownum) {
      var Brand=document.getElementById("NBrandName"+rowNum).value;
      var Location=document.getElementById("transfer_from").value;
 ...
 <body style="font-size: 11px; font-family:Tahoma, Geneva, sans-serif;">
      ...
      // echo $sql_item;
        ...
        while($row = mysqli_fetch_array($result)) {
          ...
          print '&nbsp<input type="text" name="ptransfer_no[]" id="ptransfer_no'. $row["id"].'" value="'. htmlspecialchars($row["ptransfer_no"], ENT_COMPAT).'" style="width:10%;height:30px;background:#f2f2f2;font-size:10px;" readonly/>'; 
...
// <span  class="warn warning"></span><span style="color:#ff8566;">This Update is not for Commodity.</span></td></tr>'; 
  ...
  <script type="text/javascript">
        function Delete(id)
    {
       ...
       var PL= encodeURIComponent(document.getElementById('PL'+id).value);
       var Remark_item= encodeURIComponent(document.getElementById('Remark_item'+id).value); 
       var ptransfer_no= encodeURIComponent(document.getElementById('ptransfer_no'+id).value);   

       window.location.href = "remove.php?id="+id+"&LUser="+LUser+"&transfer_from="+transfer_from+"&transfer_to="+transfer_to+"&transfer_date="+transfer_date+"&transfer_no="+transfer_no+"&BrandName="+BrandName+"&Commodity="+Commodity+"&code="+code+"&Type="+Type+"&Qty="+Qty+"&PL="+PL+"&Remark_item="+Remark_item+"&ptransfer_no="+ptransfer_no+""; 
