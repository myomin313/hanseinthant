...
?>
    ...
    $sql="SELECT Distinct product.Purchase_order_No as PL,product.transfer_no as transfer_no
    FROM product LEFT JOIN supplier ON product.SupplierName= supplier.UserId WHERE 
    product.Commodity='$CommodityN' AND product.BrandName='$BrandNameN' AND product.Branch='$transfer_from' AND code='$code' AND product.currentQty!=0 AND  product.transfer_no IS NULL"; 
    ...
    &nbsp&nbsp<input type="float" oninput="input(this)"  onblur="fncClearPl(\''.$rowNum.'\')"  name="NQty[]" id="NQty'.$rowNum.'" style="width:4%;height:30px;font-size:10px;" required=required>';
     ...
     $resultt = mysqli_query($con,$sqlt);
         ...
         print'&nbsp&nbsp<select  name="Ntransfer_no[]" onChange="showPl(\''.$rowNum.'\')"  id="Ntransfer_no'.$rowNum.'" style="width:10%;height:30px;background:#f2f2f2;font-size:10px;" readonly>';
