 ...
 if($count==1)
    ...
    //  var_dump($deliveryUSD);exit();      
     ...
     if($NQty > $rowsW['currentQty']){
      ...
      if( $Ntransfer_no != ''){
      $sqlsss="SELECT balanceDelivery,balanceReturn,balanceTransfer,balanceQty,USD,Qty,balanceUSD,currentQty FROM product Where BrandName = '$NBrandName' and Commodity ='$NCommodity' and code='$code' and Branch ='$transfer_from' and Purchase_order_No ='$txtHintss' and transfer_no='$Ntransfer_no'";
      }else{
        $sqlsss="SELECT balanceDelivery,balanceReturn,balanceTransfer,balanceQty,USD,Qty,balanceUSD,currentQty FROM product Where BrandName = '$NBrandName' and Commodity ='$NCommodity' and code='$code' and Branch ='$transfer_from' and Purchase_order_No ='$txtHintss' AND ( transfer_no IS NULL OR transfer_no='')";  
      }
   ...
   if( $Ntransfer_no != ''){
      ...
      mysqli_query($con,$sqlda); 
        ...
        $sqlda="INSERT INTO transfer(transfer_from,transfer_to,BrandName,Commodity,code,Type,Qty,transferUSD,PL,Remark_Item,transfer_date,transfer_no,ptransfer_no,Creator,CreateTime) VALUES ('$transfer_from','$transfer_to','$NBrandName','$NCommodity','$code','$NType','$NQty','$transferUSD','$txtHintss','$NRemark','$transfer_date','$transfer_no',NULL,'$LUser','$CreateTime')";
       mysqli_query($con,$sqlda);    
   }
