<?php 
    ...
    $q2 = $con->query($setutf7);*/
      ...
      $totalRate=$_GET['totalRate']; 
       ...
       $sqlA = "SELECT DISTINCT BrandName,Commodity,PL FROM job WHERE BrandName ='$BrandName' and Commodity= '$Commodity' and code= '$code' and PL = '$PL'  and Location = '$Branch'";
        $resultA=mysqli_query($con ,$sqlA); 
        $row = $resultA->fetch_assoc();
        $count=mysqli_num_rows($resultA);
        
       $sqls = "SELECT DISTINCT BrandName,Commodity,PL FROM delivery WHERE BrandName ='$BrandName' and Commodity= '$Commodity' and code= '$code' and PL = '$PL'";
      ...
      elseif($counts==1)
                  ...
                  echo "<script type='text/javascript'>window.top.location='new.php';</script>";exit;
                            ...
                            $sql="INSERT INTO product_history(input_ID,SupplierName,OSupplierName,OBrandName,OCommodity,code,Ocode,Branch,OBranch,Remarks_Item,ORemarks_Item,Packing_date,OPacking_date,Purchase_order_No,OPurchase_order_No,PO_NO,OPO_NO,Qty,OQty,totalRate,OtotalRate,Rate_Name,ORate_Name,rate,Orate,Type,OType,RecordTime,Recorder,status) 
         ...
         echo "<script type='text/javascript'>window.top.location='detail.php?SupplierID=$SupplierID&Branch=$Branch&Packing_date=$OPacking_date&PL=$PPL';</script>";exit;		
