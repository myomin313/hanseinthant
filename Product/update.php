<?php
      ...
      $totalRate=$_GET['totalRate'];
                  ...
                  $balanceReturn = $row['balanceReturn'];
                  ...
                  elseif ($Rate_Name == 'Australia') {
                   ...
                   // $balancetd = $balanceTransfer + $balanceDelivery;
                  $balancetd = $balanceTransfer + ( $balanceDelivery - $balanceReturn);
                   $balanceUSD = $itemUSD * $balancetd; 
                   
                    
                   
                   //start
                     $sqldelivery = "SELECT Location,Qty,BrandName,Commodity,PL,DO_No FROM delivery WHERE BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and PL = '$OPurchase_order_No'";
                   ...
                   //start myo min for delivery usd
                    if($dcount >1){
                         foreach($dresult as $drow) {
                            ...
                            $dsql="UPDATE delivery SET deliveryUSD='$deliveryUSD' WHERE  DO_No='$DO_No' and  Location = '$Location' and BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and PL = '$OPurchase_order_No' ";
                            mysqli_query($con ,$dsql);
                         }
                    }else{
                         $deliveryUSD = $deliveryrows['Qty'] * $itemUSD;
                         $Location = $deliveryrows['Location'];
                         $dsql="UPDATE delivery SET deliveryUSD='$deliveryUSD' WHERE  Location = '$Location' and BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and PL = '$OPurchase_order_No' ";
                         mysqli_query($con ,$dsql); 
                    }
                   //end myo min for delivery usd

                    //start myo min for transfer product usd

                      $sqltransferproduct = "SELECT Branch,Qty,BrandName,Commodity,Purchase_order_No,balanceDelivery,balanceTransfer,balanceReturn FROM product WHERE Branch!='$OBranch' and BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and Purchase_order_No = '$OPurchase_order_No'";
           
                    $transferproductresults=mysqli_query($con ,$sqltransferproduct);
                   
                   $ptcount= mysqli_num_rows($transferproductresults);
                   $transferproductrows = $transferproductresults->fetch_assoc();
                   
                   
                   $ptresult = $con->query($sqltransferproduct);

                    if($ptcount >1){
                         foreach($ptresult as $pttrow) {
                            $ptUSD = $itemUSD;
                            $pttotalUSD = $pttrow['Qty'] * $itemUSD;
                             $ptBranch=  $pttrow['Qty'];
                             
                             //start  
                              $outQty =   $pttrow['balanceTransfer'] + ($pttrow['balanceDelivery'] - $pttrow['balanceReturn']);
                              $outUSD =  $outQty * $itemUSD;
                             //end
                             
                            $pttsql="UPDATE product SET rate='$ptUSD',balanceUSD='$outUSD',USD='$ptUSD',totalRate='$pttotalUSD',totalUSD='$pttotalUSD'  WHERE   Branch='$ptBranch' and BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and Purchase_order_No = '$OPurchase_order_No'";
                            mysqli_query($con ,$pttsql);
                         }
                    }else{
                            
                            $ptUSD = $itemUSD;
                            $pttotalUSD = $transferproductrows['Qty'] * $itemUSD;
                            $ptBranch= $transferproductrows['Branch'];
                            
                             //start
                             $outQty =   $transferproductrows['balanceTransfer'] + ($transferproductrows['balanceDelivery'] - $transferproductrows['balanceReturn']);
                              $outUSD =  $outQty * $itemUSD;
                            //end
                   ...
                   //end myo min for transfer product usd

                    //start myo min for return  usd

                      $sqlreturnproduct = "SELECT id,department,Qty,BrandName,Commodity,PL FROM returns WHERE  BrandName ='$OBrandName' and Commodity= '$OCommodity' and code='$Ocode' and PL = '$OPurchase_order_No'";
                     
                    $returnresults=mysqli_query($con ,$sqlreturnproduct);
                   $retcount= mysqli_num_rows($returnresults);
                   $retrows = $returnresults->fetch_assoc();
                   
                   
                   $retresult = $con->query($sqlreturnproduct);

                    if($retcount >1){
                         foreach($retresult as $returnrow) {
                            $returnUSD = $returnrow['Qty'] * $itemUSD;
                             $retid= $returnrow['id'];
                            $retsql="UPDATE returns SET  returnUSD='$returnUSD'  WHERE id= $retid";
                            mysqli_query($con ,$retsql);
                         }
                    }else{
                             $returnUSD = $retrows['Qty'] * $itemUSD;
                             $retid= $retrows['id'];
                            $retsql="UPDATE returns SET  returnUSD='$returnUSD'  WHERE id= $retid";
                            mysqli_query($con ,$retsql);
                    }
                   //end myo min for return  usd
