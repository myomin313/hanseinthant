...
function addRow(frm) {
     ...
     $.ajax({
      ...
      success:function(data)
      {
        ...
        for(var count = 0; count < data.length; count++)
        {
          ...
          $("#commodity"+rowNum+"").html(html);
           $("#commodity"+rowNum+"").select2( {
  ...
  var row = '<p id="rowNum'+rowNum+'">';
    // row = row+'<select name="itemType[]" id="Type'+rowNum+'" style="width:10%;height:30px;" required="required"><option value="">brandname</option><option value="">commodity</option></select>';

    row = row+'<select name="BrandName[]" id="brandname'+rowNum+'" style="width:20%;height:30px;"  required="required"  placeholder="Brand" class="brandname" onchange="getCode('+rowNum+')"/></select>';

    row = row+'<select name="itemCommodity[]" id="commodity'+rowNum+'" style="width:15%;height:30px;"  required="required" onblur="fncClearBox(this,'+rowNum+',\'brand\')" onKeyUp="brand('+rowNum+')" onchange="getCode('+rowNum+')" placeholder="commodity" class="commodity"/></select>';
    
    row = row+'<select name="code[]" id="code'+rowNum+'" style="width:15%;height:30px;"  required="required"   placeholder="code no" class="code"/></select>';
    // <input type="text" name="BrandName[]"  id="search-box'+rowNum+'" style="width:20%;height:30px;" autocomplete="off" required="required" onblur="fncClearBox(this,'+rowNum+',\'brand\')" onKeyUp="brand('+rowNum+')" placeholder="Brand"/><span id="suggesstion-box'+rowNum+'"></span>&nbsp
      
    row =  row +'<select name="itemType[]" id="Type'+rowNum+'" style="width:4%;height:30px;" required="required"><option value=""></option><option value="Set">Set</option><option value="No">No</option><option value="Lot">Lot</option><option value="ft">ft</option><option value="mtrs">mtrs</option><option value="Coils">Coils</option><option value="kg">kg</option><option value="Yard">Yard</option></select>&nbsp<select name="Rate_Name[]" id="Rate_Name'+rowNum+'"  style="width:5%;height:30px;"><option value=""></option><option value="USD">USD</option><option value="Myanmar">Myanmar</option><option value="Singapore">Singapore</option><option value="Europe">Europe</option><option value="Thailand">Thailand</option><option value="British">British</option><option value="Japan">Japan</option><option value="Australia">Australia</option></select>&nbsp<input type="text" name="rate[]" id="rate'+rowNum+'" style="width:8%;height:30px;" placeholder="1 per/rate" onkeyup="out('+rowNum+')">&nbsp<input type="float" name="itemQty[]" id="itemQty'+rowNum+'" style="width:5%;height:30px;" placeholder="Qty" oninput="input(this)" onkeyup="out('+rowNum+')"/>&nbsp<input type="text" name="totalRate[]" id="totalRate'+rowNum+'" style="width:8%;height:30px;" placeholder="rate" onkeyup="out('+rowNum+')">&nbsp<input type="text" name="itemRemarks_Item[]" style="width:25%;height:30px;" placeholder="Remark"/>&nbsp&nbsp&nbsp<input type="button" value="remove"  onclick="removeRow('+rowNum+');" class="removeitem"></p>';
...
<!-- get code -->
  <script>
    function getCode(str) {
      var BrandName=document.getElementById("brandname"+rowNum).value;
      var Commodity=document.getElementById("commodity"+rowNum).value;
      // alert(Brand);
        if (str == "") {
            document.getElementById("code"+rowNum).innerHTML = "";
            return;
        } else { 
            ...
            xmlhttp.onreadystatechange = function() {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    document.getElementById("code"+rowNum).innerHTML = xmlhttp.responseText;
                }
            }
// getuser.php is seprate php file. q is parameter 
            //xmlhttp.open("GET","get-commodity.php?q="+str+"&rowNum="+rowNum+"&Brand="+Brand+"&JOB_Nos="+JOB_Nos+"&Location="+Location+"",true);
            xmlhttp.open("GET","code.php?q="+encodeURIComponent(str)+"&rowNum="+encodeURIComponent(rowNum)+"&BrandName="+encodeURIComponent(BrandName)+"&Commodity="+encodeURIComponent(Commodity)+"",true);
            xmlhttp.send();
        }
    }
                     ...
                     print '<tr><td style="width:7%;">Supplier Name</td><td> <select name="SupplierName" id="SupplierName" style="width:20%;height:30px;" required="required">';
                        ...
                        print '<option value="'. $UserId.'"';
                         ...
                         print '>'. $SupplierName.'</option>';
                          ...
                          print '<tr><td style="width:7%;">Branch Name</td><td> <select name="Branch" id="Branch" style="width:20%;height:30px;" required="required">';
                          print'<option value =""></option>';
...
$("#SupplierName").select2( {
  ...
  allowClear: true
   ...
   //$('.brandname:first').select2();
 // $('.commodity:first').select2();
//  $(".brandname").select2( {
//   placeholder: "Select Brandname",
//   allowClear: true
//   } );
