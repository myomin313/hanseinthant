...
if ($JOB_Noss == '') {
    ...
    LEFT JOIN supplier
    ON product.SupplierName= supplier.UserId Where 
    product.Commodity='$CommodityN' and product.BrandName='$BrandNameN'  and Branch='$Location' and currentQty!=0";    
}else{
    $sql="SELECT Distinct job.PL as PL
    FROM product 
    LEFT JOIN job ON product.Commodity=job.Commodity 
    LEFT JOIN supplier
    ON product.SupplierName= supplier.UserId Where 
    product.Commodity='$CommodityN' and product.BrandName='$BrandNameN' and job.Job_No='$JOB_Noss' and Branch='$Location' and currentQty!=0";}
    // echo $sql;
