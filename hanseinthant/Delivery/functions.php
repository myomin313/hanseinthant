...
// output headers so that the file is downloaded rather than displayed
 ...
 fputcsv($output,  array('id','Delivery Date','DO No','Location','Customer','BrandName','Commodity','Qty','Type','Remark_item','PL','Creator','CreateTime'));
...
// $sql = "SELECT id,Delivery_date,DO_No,Location,Customer,BrandName,Commodity,Qty,Type,Remark_item,PL,Creator,CreateTime from delivery ";
   $sWhere = "";
   $sql=" SELECT delivery.id,delivery.Delivery_date,delivery.DO_No,delivery.Location,customer.customerName,delivery.BrandName,delivery.Commodity,delivery.Qty,delivery.Type,delivery.Remark_item,delivery.PL,delivery.Creator,delivery.CreateTime 
          FROM delivery 
          LEFT JOIN customer ON customer.UserId= delivery.Customer"; 

             if(!empty($_POST['keyword'])){
                   $keyword = $_POST['keyword'];
                $sWhere =$sWhere . " AND (customer.CustomerName= '".$keyword."'OR delivery.BrandName= '".$keyword."'OR delivery.Location= '".$keyword."'OR delivery.DO_No= '".$keyword."'OR delivery.JOB_No= '".$keyword."'OR delivery.Commodity= '".$keyword."')";
                }
