       ...
       foreach($idarray as $id) {
         ...
         $row = $results->fetch_assoc();
           ...
           $ORemarks_Item = mysqli_real_escape_string($con,$row['Remarks_Item']);
            ...
            $sqlA = "SELECT DISTINCT BrandName,Commodity,PL FROM job WHERE BrandName ='$OBrandName' and Commodity= '$OCommodity' and PL = '$OPL_NO'  and Location = '$OBranch'";
        $resultA=mysqli_query($con ,$sqlA); 
        $row = $resultA->fetch_assoc();
        $count=mysqli_num_rows($resultA);
        
       $sqls = "SELECT DISTINCT BrandName,Commodity,PL FROM delivery WHERE BrandName ='$OBrandName' and Commodity= '$OCommodity' and PL = '$OPL_NO'";
        ...
        $counts=mysqli_num_rows($results); 
         ...
         echo "<script type='text/javascript'>window.top.location='detail.php?SupplierID=$SupplierID&Date=$Date&PL=$PL';</script>";exit;  
           ...
           mysqli_query($con,$sql);
             ...
             echo "<script type='text/javascript'>window.top.location='detail.php?SupplierID=$SupplierID&PL=$OPL_NO';</script>";  
