...
<div id="outer-container">
    ...
    <table id="emp_table" border="0" class="resultGridTable" style="width: auto;margin-top: 1%;font-size:10px;">
        ...
        <?php
                        ...
                        $sqlA="SELECT code.id,code.code,brandname.brandname,commodity.commodity,code.updated_by FROM code LEFT JOIN brandname
                        ON brandname.id= code.brandname LEFT JOIN commodity
                        ON commodity.id= code.commodity  WHERE 1=1 ";
                            if (isset($_POST['keyword'])) {
                              $keyword = $_POST['keyword']; // DO NOT FORGET ABOUT STRING SANITIZATION
                              $sqlA =$sqlA." AND code LIKE '%$keyword%' OR brandname.brandname LIKE '%$keyword%' OR commodity.commodity LIKE '%$keyword%'";
                            }  
                        
                             if($_GET['namelist'] <> ''){
                               $sqlA =$sqlA." AND code= '".$_GET['namelist']."'  OR brandname.brandname= '".$_GET['namelist']."' OR commodity.commodity= '".$_GET['namelist']."'";
                             }                          
