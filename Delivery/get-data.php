...
if ($JOB_Noss == '') {
    ...
    LEFT JOIN supplier
    ON product.SupplierName= supplier.UserId Where 
    product.Commodity='$CommodityN' and product.BrandName='$BrandNameN'  and product.Branch='$Location' and currentQty!=0 and product.transfer_no IS NULL";    
}else{
    $sql="SELECT Distinct job.PL as PL
    FROM product 
    LEFT JOIN job ON product.Commodity=job.Commodity 
    LEFT JOIN supplier
    ON product.SupplierName= supplier.UserId Where 
    job.Commodity='$CommodityN' and job.BrandName='$BrandNameN' and job.Job_No='$JOB_Noss' and job.Location='$Location' and product.transfer_no IS NULL";
