 ...
 <body style="font-size: 10px; font-family:Tahoma, Geneva, sans-serif;">
   ...
   Do No: <select name="DO_No" id="search_DO_No" style="width:15%;height:30px;">;
                     ...
                     <?php
                          ...
                          $querypur ="SELECT DISTINCT DO_No FROM delivery ORDER BY DO_No";
...
PL NO: <select name="PL" id="search_PL" style="width:15%;height:30px;">;
        ...
        <input type="button" id="deliverySearch" value="search">
    ...
    <table id="emp_table" border="0" class="emp" style="width: auto;margin-top: 1%;font-size:10px;">
        <tr class="tr_header" style="background-color:#3385ff;font-family:Tahoma, Geneva, sans-serif;font-size: 12px;">
                  ...
                  if ($Authority == 'Accountant') {          
                      print '<th rowspan="2" style="word-wrap:break-word;">Delivery USD</th>';
                   }                
                  ?>
                  <th rowspan="2" style="font-weight:normal; word-break: break-all; ">PL</th>
                  <th rowspan="2" style="font-weight:normal; word-break: break-all; ">Transfer No</th> 
       ...
       //  $sql = "SELECT Delivery_date, count(*) as count_rows FROM delivery ";   
         ...
         *
         ...
         (
          ...
          $sqlA="SELECT d.transfer_no as transfer_no,d.id as id,branch.branch as branch_name,d.Customer as CustomerID,co.code as code_no,d.Delivery_date as Delivery_date,d.DO_No as DO_No,d.JOB_No as jJob,c.CustomerName as Customers,c.UserId as cid,d.Type as Type,d.Location as Location,d.PL as PL,brandname.brandname as BrandName,commodity.commodity as Commodity,d.Qty as Qty,d.Remark_item as remark,d.returnQty as rQty,d.deliveryUSD as deliveryUSD FROM delivery d
          JOIN brandname ON d.BrandName= brandname.id
          JOIN commodity ON d.Commodity= commodity.id
          JOIN branch ON branch.id= d.Location
          LEFT JOIN customer c ON c.UserId= d.Customer
          LEFT JOIN code co ON co.id= d.code 
          Where " ;  
          // if($Branches == 1){
...
//Where d.Location='Mandalay Show Room'
                ...
                // develop by Winsan
                            ...
                            //   $sqlA =$sqlA." AND (customer.CustomerName LIKE '%$keyword%' OR brandname.brandname= '".$_POST['keyword']." OR commodity.commodity= '".$_POST['keyword']."' OR delivery.JOB_No LIKE '%$keyword%'OR delivery.DO_No LIKE '%$keyword%'OR delivery.Location LIKE '%$keyword%'OR delivery.PL LIKE '%$keyword%' )";
