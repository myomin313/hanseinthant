    ...
    //     	}
        ...
        //	$commodity =htmlentities($_POST['commodity']);
        	$commodity = mysqli_real_escape_string($con,$_POST['commodity']);
        	$LUser=$_POST['LUser']; 

        $sql = "SELECT Distinct commodity FROM commodity  WHERE commodity='$commodity'";
        // echo $sql;
      ...
      if($count==1)
                  {
                   echo"<script>alert('commodity already exist.Please try again!');</script>";  
                   echo "<script type='text/javascript'>window.top.location='new.php';</script>";exit;
       
                  }  
             
      else{
	        $sql = "INSERT INTO commodity (commodity,updated_by) VALUES ('$commodity','$LUser')";
